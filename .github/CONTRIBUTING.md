...
AI-assisted contributions are welcome. Tools like Copilot and other LLMs can be genuinely useful, and there's no expectation that you avoid them.

What matters is thoughtful engagement with your own submission. Before opening a PR, make sure you understand the change, that it actually solves the problem, and that you've reviewed and tested it as if you'd written it by hand. Please don't open PRs that are unreviewed model output — they take longer to review than they took to generate, and that asymmetry isn't fair to maintainers. A short note on how you validated the change goes a long way.
...
```bash
rm -rf ~/.terraform.d/plugins/registry.terraform.io/SamuZad/
```
